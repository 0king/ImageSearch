...
public class SpanningGridLayoutManager extends GridLayoutManager {
    ...
    private RecyclerView.LayoutParams spanLayoutSize(RecyclerView.LayoutParams layoutParams){
        if(getOrientation() == HORIZONTAL){
            ...
            layoutParams.width = getHorizontalSpace()/getSpanCount();
            layoutParams.height = getHorizontalSpace()/getSpanCount();
        }
        else if(getOrientation() == VERTICAL){
            //layoutParams.height = (int) Math.round(getVerticalSpace() / Math.ceil(getItemCount() / getSpanCount()));
            layoutParams.width = getVerticalSpace()/getSpanCount();
            layoutParams.height = getVerticalSpace()/getSpanCount();
        }
        return layoutParams;
    }
